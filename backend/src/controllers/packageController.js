...
export const getPackages = async (req, res, next) => {
  try {
    ...
    let packages = await TravelPackage.find(query).limit(Number(limit)).lean();
    if (!packages || packages.length === 0) {
      packages = DEFAULT_PACKAGES.filter((p) =>
        !destination || p.destination.toLowerCase().includes(destination.toLowerCase())
      );
    }

    return sendSuccess(res, packages, "Travel packages retrieved.");
  } catch (error) {
    next(error);
  }
};
...
export const getPackageById = async (req, res, next) => {
  try {
    const { id } = req.params;
    let pkg = await TravelPackage.findOne({
      $or: [{ _id: id.match(/^[0-9a-fA-F]{24}$/) ? id : null }, { slug: id }]
    }).lean();

    if (!pkg) {
      pkg = DEFAULT_PACKAGES.find((p) => p.slug === id || p._id === id);
    }
    ...
    return sendSuccess(res, pkg, "Package details retrieved.");
  } catch (error) {
    next(error);
  }
};
...
export const getPackagesByDestination = async (req, res, next) => {
  try {
    ...
    let packages = await TravelPackage.find({
      $or: [{ destination: regex }, { state: regex }, { placesCovered: { $in: [regex] } }]
    }).lean();

    if (!packages || packages.length === 0) {
      packages = DEFAULT_PACKAGES.filter(
        (p) =>
    ...
    if (packages.length === 0) {
      packages = DEFAULT_PACKAGES.slice(0, 3);
    }

    return sendSuccess(res, packages, `Packages for ${dest} retrieved.`);
  } catch (error) {
    next(error);
  }
};
